...
// Định danh cho xe đạp và chỗ đỗ xe (có thể tùy chỉnh)
...
// Các chân kết nối RGB LED cho XE ĐẠP (common anode: mức thấp = sáng)
const int pinR = D5;
const int pinG = D6;
const int pinB = D7;

// Các chân kết nối RGB LED cho CHỖ ĐỂ XE (common anode: mức thấp = sáng) - CHỈNH SỬA ĐÂY
const int pinParkingR = D2;
const int pinParkingG = D3;
const int pinParkingB = D4;

// Chân loa - Dùng chung cho cả xe đạp và chỗ đỗ xe trong ví dụ này
...
callback(char* topic, byte* payload, unsigned int length)
{
    ...
    if (topicStr.startsWith("bike/status/")) {
        ...
        if (topicBikeId == String(localBikeId)) {
            ...
            // **CHỈNH SỬA ĐÂY: Thêm kích thước cho StaticJsonDocument**
